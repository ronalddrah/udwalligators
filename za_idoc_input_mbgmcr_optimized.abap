...
* local data
  TYPES: BEGIN OF ty_mara_buff,
           matnr TYPE mara-matnr, mtart TYPE mara-mtart, labor TYPE mara-labor, meins TYPE mara-meins,
         END OF ty_mara_buff,
         tt_mara_buff TYPE HASHED TABLE OF ty_mara_buff WITH UNIQUE KEY matnr.
  TYPES: BEGIN OF ty_ekko_buff,
           ebeln TYPE ekko-ebeln, bsart TYPE ekko-bsart, lifnr TYPE ekko-lifnr, bedat TYPE ekko-bedat, bukrs TYPE ekko-bukrs,
         END OF ty_ekko_buff,
         tt_ekko_buff TYPE HASHED TABLE OF ty_ekko_buff WITH UNIQUE KEY ebeln.
  TYPES: BEGIN OF ty_ekpo_buff,
           ebeln TYPE ekpo-ebeln, ebelp TYPE ekpo-ebelp, weora TYPE ekpo-weora, loekz TYPE ekpo-loekz,
           insmk TYPE ekpo-insmk, werks TYPE ekpo-werks, matnr TYPE ekpo-matnr, bstyp TYPE ekpo-bstyp,
           elikz TYPE ekpo-elikz, bednr TYPE ekpo-bednr, afnam TYPE ekpo-afnam,
         END OF ty_ekpo_buff,
         tt_ekpo_buff TYPE HASHED TABLE OF ty_ekpo_buff WITH UNIQUE KEY ebeln ebelp.
  TYPES: BEGIN OF ty_likp_buff,
           vbeln TYPE likp-vbeln, vbtyp TYPE likp-vbtyp, lfart TYPE likp-lfart,
         END OF ty_likp_buff,
         tt_likp_buff TYPE HASHED TABLE OF ty_likp_buff WITH UNIQUE KEY vbeln.
  TYPES: BEGIN OF ty_lips_buff,
           vbeln TYPE lips-vbeln, posnr TYPE lips-posnr, vgbel TYPE lips-vgbel, vgpos TYPE lips-vgpos,
           matnr TYPE lips-matnr, werks TYPE lips-werks, lgort TYPE lips-lgort, uecha TYPE lips-uecha,
           charg TYPE lips-charg, meins TYPE lips-meins, vrkme TYPE lips-vrkme, wbsta TYPE lips-wbsta,
         END OF ty_lips_buff,
         tt_lips_buff TYPE SORTED TABLE OF ty_lips_buff WITH UNIQUE KEY vbeln posnr
                      WITH NON-UNIQUE SORTED KEY po_ref COMPONENTS vgbel vgpos.
  TYPES: tt_ekbe_buff TYPE SORTED TABLE OF ekbe WITH NON-UNIQUE KEY ebeln ebelp matnr werks xblnr.

  DATA: lt_mara_buff TYPE tt_mara_buff,
        lt_ekko_buff TYPE tt_ekko_buff,
        lt_ekpo_buff TYPE tt_ekpo_buff,
        lt_likp_buff TYPE tt_likp_buff,
        lt_lips_buff TYPE tt_lips_buff,
        lt_ekbe_buff TYPE tt_ekbe_buff.
...
* PRE-FETCH DATA
  LOOP AT idoc_data ASSIGNING FIELD-SYMBOL(<ls_pfc>).
    IF <ls_pfc>-segnam = 'E1BP2017_GM_ITEM_CREATE'.
      DATA(ls_it_pfc) = CONV e1bp2017_gm_item_create( <ls_pfc>-sdata ).
      IF ls_it_pfc-material IS NOT INITIAL. INSERT ls_it_pfc-material INTO TABLE lt_matnr_keys. ENDIF.
      IF ls_it_pfc-po_number IS NOT INITIAL. INSERT ls_it_pfc-po_number INTO TABLE lt_ebeln_keys. ENDIF.
      IF ls_it_pfc-deliv_numb_to_search IS NOT INITIAL. INSERT ls_it_pfc-deliv_numb_to_search INTO TABLE lt_vbeln_keys. ENDIF.
    ENDIF.
  ENDLOOP.

  IF lt_matnr_keys IS NOT INITIAL.
    SELECT matnr, mtart, labor, meins FROM mara FOR ALL ENTRIES IN @lt_matnr_keys WHERE matnr = @lt_matnr_keys-table_line INTO TABLE @lt_mara_buff.
  ENDIF.
  IF lt_ebeln_keys IS NOT INITIAL.
    SELECT ebeln, bsart, lifnr, bedat, bukrs FROM ekko FOR ALL ENTRIES IN @lt_ebeln_keys WHERE ebeln = @lt_ebeln_keys-table_line INTO TABLE @lt_ekko_buff.
    SELECT ebeln, ebelp, weora, loekz, insmk, werks, matnr, bstyp, elikz, bednr, afnam FROM ekpo FOR ALL ENTRIES IN @lt_ebeln_keys WHERE ebeln = @lt_ebeln_keys-table_line INTO TABLE @lt_ekpo_buff.
    SELECT * FROM ekbe FOR ALL ENTRIES IN @lt_ebeln_keys WHERE ebeln = @lt_ebeln_keys-table_line INTO TABLE @lt_ekbe_buff.
  ENDIF.
  IF lt_vbeln_keys IS NOT INITIAL.
    SELECT vbeln, vbtyp, lfart FROM likp FOR ALL ENTRIES IN @lt_vbeln_keys WHERE vbeln = @lt_vbeln_keys-table_line INTO TABLE @lt_likp_buff.
    SELECT vbeln, posnr, vgbel, vgpos, matnr, werks, lgort, uecha, charg, meins, vrkme, wbsta FROM lips FOR ALL ENTRIES IN @lt_vbeln_keys WHERE vbeln = @lt_vbeln_keys-table_line INTO TABLE @lt_lips_buff.
  ENDIF.
...
*         In case of YIDV delivery - get PO number if not included in Idoc
            ...
            CLEAR hi_weora.
            hi_weora = VALUE #( lt_ekpo_buff[ ebeln = goodsmvt_item-po_number ebelp = goodsmvt_item-po_item ]-weora OPTIONAL ). IF hi_weora <> 'X'. sy-subrc = 4. ELSE. sy-subrc = 0. ENDIF.
            IF sy-subrc IS INITIAL.
...
* POS inbound delivery
            IF NOT goodsmvt_item-deliv_numb_to_search IS INITIAL.
              gf_mtart = VALUE #( lt_mara_buff[ matnr = goodsmvt_item-material ]-mtart OPTIONAL ).
...
* Reversal will be created manually
              IF VALUE #( lt_mara_buff[ matnr = goodsmvt_item-material ]-labor OPTIONAL ) = co_labor_etikett. sy-subrc = 0. ELSE. sy-subrc = 4. ENDIF.
